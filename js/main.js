  ...
  for (let i = 0; i < servantList.length; i++){
    if ( $('#servant').val() == servantList[i].id ){
		...
		if (servantList[i].oc){
			...
			oc=servantList[i].oc[0]
		} else {
			$('#ocLevel').val(0).attr('disabled','disabled');
		}
		
		$('input[name=cardoptions]').on('change', function(){
			cardColor = $('input[name=cardoptions]:checked').val()
			switch(true){
				case (servantList[$('#servant').val()-1].id==312 && cardColor=="buster"):
					multi = servantList[i].np2[0].slice(1);
					break;
				case (servantList[$('#servant').val()-1].id==11 && cardColor=="arts"):
					multi = servantList[i].np2[0].slice(1);
					break;
				case (servantList[$('#servant').val()-1].id==268 && cardColor=="buster"):
					multi = servantList[i].np2[0].slice(1);
					break;
				case (servantList[$('#servant').val()-1].id==268 && cardColor=="quick"):
					multi = servantList[i].np3[0].slice(1);
					break;
				default:
					multi = servantList[i].np[0].slice(1);
					break;
			}
			$('#NP').val( Math.round(Number( multi[$('#npLevel').val()]/10) + Number(oc[$('#ocLevel').val()]/10)));
		})
        ...
        $('#npButton').on('change', function(){
          if ($(this).is(':checked')){
			...
			if (servantList[i].oc) if(servantList[i].oc[1]){
				oc=servantList[i].oc[1]
			}
            $('#NP').val( Math.round(Number( multi[$('#npLevel').val()]/10) + Number(oc[$('#ocLevel').val()]/10)));
          }
          else {
        ...
        $('#ocButton').on('change', function(){
          if ($(this).is(':checked')){
            oc = servantList[i].overcharge;
            $('#ocLevel').val(0).removeAttr('disabled','disabled');
            $('#NP').val( Math.round(Number( multi[$('#npLevel').val()]) + Number(oc[$('#ocLevel').val()]/10)));
          }
          else {
            oc = [0,0,0,0,0];
            $('#ocLevel').val(0).attr('disabled','disabled');
